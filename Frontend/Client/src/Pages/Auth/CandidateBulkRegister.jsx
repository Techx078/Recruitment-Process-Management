...
export default function CandidateBulkRegister() {
  ...
  const sendToBackend = async () => {
    try {
      let token = localStorage.getItem("token");
      if (!token) {
        toast.error("login as a recruiter!");
        Navigate("/login");
        return;
      }
      //register candidates
        ...
        <>
          ...
          <div className="mb-6 p-4 border rounded-lg bg-gray-100">
            ...
            <pre className="text-sm bg-white p-3 rounded border mb-3">
              ...
              <div>
                | ResumePath(google drive links) | Domain |
                DomainExperienceYears |
              </div>
            </pre>

            <pre className="text-sm bg-white p-3 rounded border mb-3">
              Educations.xlsx: Email | Degree | University | College |
              PassingYear | Percentage(in integer)
            </pre>
          ...
          {finalJson.length > 0 && (
            <>
              ...
              <p className="text-md text-gray-600 mb-2">
                Note: Highlighted rows indicate candidates that already existed
        ...
        <>
          <div className="mt-8 border-t pt-6">
            ...
            <button
              ...
              disabled={jobLoading}
              className="w-full mt-4 bg-gray-600 hover:bg-gray-700 py-2 rounded-lg transition-all"
            >
